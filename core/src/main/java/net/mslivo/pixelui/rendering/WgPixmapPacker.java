...
/** Packs {@link Pixmap pixmaps} into one or more {@link WgPixmapPacker.Page pages} to generate an atlas of pixmap instances. Provides means to
 ...
 * How bin packing is performed can be customized via {@link WgPixmapPacker.PackStrategy}.
 * <p>
 ...
 * @author Nathan Sweet
 * @author Rob Rendell */
public class WgPixmapPacker implements Disposable {
    boolean packToTexture;
    boolean disposed;
    int pageWidth, pageHeight;
    WGPUTextureFormat pageFormat;
    int padding;
    boolean duplicateBorder;
    boolean stripWhitespaceX, stripWhitespaceY;
    int alphaThreshold;
    Color transparentColor = new Color(0f, 0f, 0f, 0f);
    final Array<WgPixmapPacker.Page> pages = new Array();
    WgPixmapPacker.PackStrategy packStrategy;

    static Pattern indexPattern = Pattern.compile("(.+)_(\\d+)$");

    /** Uses {@link WgPixmapPacker.GuillotineStrategy}.
     * @see WgPixmapPacker#PixmapPacker(int, int, Format, int, boolean, boolean, boolean, WgPixmapPacker.PackStrategy) */
    public WgPixmapPacker(int pageWidth, int pageHeight, WGPUTextureFormat pageFormat, int padding, boolean duplicateBorder) {
        this(pageWidth, pageHeight, pageFormat, padding, duplicateBorder, false, false, new WgPixmapPacker.GuillotineStrategy());
    }
    ...
    public WgPixmapPacker(int pageWidth, int pageHeight, WGPUTextureFormat pageFormat, int padding, boolean duplicateBorder,
                          boolean stripWhitespaceX, boolean stripWhitespaceY, WgPixmapPacker.PackStrategy packStrategy) {
    ...
    public synchronized WgPixmapPacker.PixmapPackerRectangle pack (Pixmap image) {
     ...
     *            name. */
    public synchronized WgPixmapPacker.PixmapPackerRectangle pack (String name, Pixmap image) {
        if (disposed) return null;
        if (name != null && getRect(name) != null)
            throw new GdxRuntimeException("Pixmap has already been packed with name: " + name);

        WgPixmapPacker.PixmapPackerRectangle rect;
        Pixmap pixmapToDispose = null;
        if (name != null && name.endsWith(".9")) {
            rect = new WgPixmapPacker.PixmapPackerRectangle(0, 0, image.getWidth() - 2, image.getHeight() - 2);
            ...
            if (stripWhitespaceX || stripWhitespaceY) {
                ...
                rect = new WgPixmapPacker.PixmapPackerRectangle(0, 0, newWidth, newHeight, left, top, originalWidth, originalHeight);
            } else {
                rect = new WgPixmapPacker.PixmapPackerRectangle(0, 0, image.getWidth(), image.getHeight());
            }
        }
        ...
        WgPixmapPacker.Page page = packStrategy.pack(this, name, rect.bounds);
        if (name != null) {
    ...
    /** @return the {@link WgPixmapPacker.Page} instances created so far. If multiple threads are accessing the packer, iterating over the pages
     *         must be done only after synchronizing on the packer. */
    public Array<WgPixmapPacker.Page> getPages () {
        return pages;
    }

    /** @param name the name of the image
     * @return the rectangle for the image in the page it's stored in or null */
    public synchronized WgPixmapPacker.PixmapPackerRectangle getRect (String name) {
        for (WgPixmapPacker.Page page : pages) {
            WgPixmapPacker.PixmapPackerRectangle rect = page.rects.get(name);
    ...
    public synchronized WgPixmapPacker.Page getPage (String name) {
        for (WgPixmapPacker.Page page : pages) {
    ...
    /** Calls {@link WgPixmapPacker.Page#updateTexture(TextureFilter, TextureFilter, boolean) updateTexture} for each page and adds a region to
    ...
    /** Calls {@link WgPixmapPacker.Page#updateTexture(TextureFilter, TextureFilter, boolean) updateTexture} for each page. */
    ...
    public WGPUTextureFormat getPageFormat () {
        return pageFormat;
    }

    public void setPageFormat (WGPUTextureFormat pageFormat) {
        this.pageFormat = pageFormat;
    }
    ...
    static public class Page {
        OrderedMap<String, WgPixmapPacker.PixmapPackerRectangle> rects = new OrderedMap();
        Pixmap image;
        WgTexture texture;
        final Array<String> addedRects = new Array();
        boolean dirty;

        /** Creates a new page filled with the color provided by the {@link WgPixmapPacker#getTransparentColor()} */
        public Page (WgPixmapPacker packer) {
            image = new Pixmap(packer.pageWidth, packer.pageHeight, Format.RGBA8888);
        ...
        public OrderedMap<String, WgPixmapPacker.PixmapPackerRectangle> getRects () {
            return rects;
        }
        ...
        public boolean updateTexture (TextureFilter minFilter, TextureFilter magFilter, boolean useMipMaps) {
            if (texture != null) {
                ...
                texture = new WgTexture(new PixmapTextureData(image, image.getFormat(), useMipMaps, false, true)) {
                    @Override
                    public void dispose () {
                        super.dispose();
                        image.dispose();
    ...
    public interface PackStrategy {
        ...
        WgPixmapPacker.Page pack (WgPixmapPacker packer, String name, WgPixmapPacker.Bounds bounds);
    }
     ...
     * @author Nathan Sweet
     * @author Rob Rendell */
    static public class GuillotineStrategy implements WgPixmapPacker.PackStrategy {
        ...
        public WgPixmapPacker.Page pack (WgPixmapPacker packer, String name, WgPixmapPacker.Bounds bounds) {
            WgPixmapPacker.GuillotineStrategy.GuillotinePage page;
            if (packer.pages.size == 0) {
                // Add a page if empty.
                page = new WgPixmapPacker.GuillotineStrategy.GuillotinePage(packer);
                packer.pages.add(page);
            } else {
                // Always try to pack into the last page.
                page = (WgPixmapPacker.GuillotineStrategy.GuillotinePage)packer.pages.peek();
            }

            int padding = packer.padding;
            bounds.width += padding;
            bounds.height += padding;
            WgPixmapPacker.GuillotineStrategy.Node node = insert(page.root, bounds);
            if (node == null) {
        ...
        private WgPixmapPacker.GuillotineStrategy.Node insert (WgPixmapPacker.GuillotineStrategy.Node node, WgPixmapPacker.Bounds rect) {
            if (!node.full && node.leftChild != null && node.rightChild != null) {
                WgPixmapPacker.GuillotineStrategy.Node newNode = insert(node.leftChild, rect);
                ...
                node.leftChild = new WgPixmapPacker.GuillotineStrategy.Node();
                node.rightChild = new WgPixmapPacker.GuillotineStrategy.Node();
        ...
        static final class Node {
            public WgPixmapPacker.GuillotineStrategy.Node leftChild;
            public WgPixmapPacker.GuillotineStrategy.Node rightChild;
            public final WgPixmapPacker.Bounds rect = new WgPixmapPacker.Bounds();
            public boolean full;
        }

        static class GuillotinePage extends WgPixmapPacker.Page {
            WgPixmapPacker.GuillotineStrategy.Node root;

            public GuillotinePage (WgPixmapPacker packer) {
                super(packer);
                root = new WgPixmapPacker.GuillotineStrategy.Node();
     ...
     * @author Nathan Sweet */
    static public class SkylineStrategy implements WgPixmapPacker.PackStrategy {
        Comparator<Pixmap> comparator;
        ...
        public WgPixmapPacker.Page pack (WgPixmapPacker packer, String name, WgPixmapPacker.Bounds rect) {
            int padding = packer.padding;
            ...
            for (int i = 0, n = packer.pages.size; i < n; i++) {
                WgPixmapPacker.SkylineStrategy.SkylinePage page = (WgPixmapPacker.SkylineStrategy.SkylinePage)packer.pages.get(i);
                Row bestRow = null;
            ...
            // Fit in new page.
            WgPixmapPacker.SkylineStrategy.SkylinePage page = new WgPixmapPacker.SkylineStrategy.SkylinePage(packer);
            packer.pages.add(page);
            Row row = new Row();
        ...
        static class SkylinePage extends WgPixmapPacker.Page {
            Array<WgPixmapPacker.SkylineStrategy.Row> rows = new Array();

            public SkylinePage (WgPixmapPacker packer) {
                super(packer);
    ...
    /** @see WgPixmapPacker#setTransparentColor(Color color) */
    public Color getTransparentColor () {
        return this.transparentColor;
    }

    /** Sets the default <code>color</code> of the whole {@link WgPixmapPacker.Page} when a new one created. Helps to avoid texture
     * bleeding or to highlight the page for debugging.
     * @see WgPixmapPacker.Page#Page(WgPixmapPacker packer) */
    ...
    public static class PixmapPackerRectangle {
        public WgPixmapPacker.Page page;
        public int[] splits;
        public int[] pads;
        public int offsetX, offsetY;
        public int originalWidth, originalHeight;
        public WgPixmapPacker.Bounds bounds;

        PixmapPackerRectangle (int x, int y, int width, int height) {
            bounds = new WgPixmapPacker.Bounds(x, y, width, height);
            this.offsetX = 0;
            this.offsetY = 0;
